...
std::mutex g_mutex;
int g_pending_stance = -1;  // -1 = none, 0 = Standing, 1 = Walking, 2 = Ragdoll
...
/** Returns 0/1/2 for standing/walking/ragdoll, or -1 if body is invalid. */
...
void http_control_start(int port) {
  ...
  g_server->Patch("/stance", [](const httplib::Request& req, httplib::Response& res) {
    ...
    if (stance >= 0) {
      ...
      res.status = 400;
      res.set_content("{\"error\":\"bad body: use {\\\"stance\\\": \\\"standing\\\" | \\\"walking\\\" | \\\"ragdoll\\\"}\"}", "application/json");
    }
  });
...
void http_control_apply_pending_stance(ControllerState& ctrl_state) {
  std::lock_guard<std::mutex> lock(g_mutex);
  if (g_pending_stance >= 0 && g_pending_stance <= 2) {
    ctrl_state.mode = static_cast<MotionMode>(g_pending_stance);
    g_pending_stance = -1;
  }
}
